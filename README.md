...
[![DMG Installer](images/dmg-installer-preview.png)](https://github.com/MikeT18/macos-gemini-overlay/releases/download/0.0.5/macos-gemini-overlay.dmg)
...
```bash
git clone https://github.com/MikeT18/macos-gemini-overlay.git
...
If you want to build the DMG installer yourself (e.g., to include your own changes):
...
2. **Configure the build**:
   Enter the `dmg-builder` folder and create your config file.
   ```bash
   cd dmg-builder
   cp sample-config.sh config.sh
   # Open config.sh in a text editor and update with your Apple Developer Team ID and credentials
   ```
...
3. **Run the build script**:
   ...
   ./build.sh
   ```
   *Note: This script attempts to use your system's python3.*

4. **Install**:
   If successful, you will find `macos-gemini-overlay.dmg` in the `dmg-builder` folder. Open it to install.
